...
export class DuckDBAdapter implements DatabaseAdapter {
  ...
  private tableName = 'insurance_records' // 默认表名
  private fullTableName = 'insurance_records' // 完整表名（包含 schema）
  ...
  async initialize(file: File): Promise<void> {
    try {
      ...
      // 5. 打开数据库文件
      // DuckDB-WASM 使用 ATTACH 语法（不需要 DATABASE 关键字）
      try {
        await this.conn.query(`ATTACH '${file.name}' AS imported_db`)
        console.log('[DuckDB] 数据库文件已附加为 imported_db')
      } catch (e) {
        console.warn('[DuckDB] 附加数据库失败，将尝试直接读取:', e)
        // 如果附加失败，表可能已经在 main schema 中
      }

      // 6. 验证表是否存在 - 检查所有 schema
      ...
      console.log(`[DuckDB] 找到表 ${this.tableName}，位于 schema: ${schema}`)
      console.log(`[DuckDB] 使用完整表名: ${this.fullTableName}`)
  ...
  async getStatistics() {
    ...
    try {
      ...
      `)
      const uniqueWeeks = weeksResult.toArray().map(r => Number(r.week_number))
