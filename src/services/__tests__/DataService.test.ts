...
describe("DataService", () => {
  describe("filter", () => {
    ...
    test("应正确筛选多个条件（AND逻辑）", () => {
      ...
      expect(result[0].insurance_type).toBe("商业险");
      expect(result[0].week_number).toBe(11);
    });

    test("应支持排除特定筛选键", () => {
      const mockData = [
        createMockRecord({ policy_start_year: 2024, insurance_type: "商业险" }),
        createMockRecord({ policy_start_year: 2025, insurance_type: "交强险" }),
      ];
  ...
  describe("groupBy", () => {
    test("应正确按维度分组", () => {
      ...
      expect(result.get("商业险")).toHaveLength(2);
      expect(result.get("交强险")).toHaveLength(1);
    });
  });
  ...
  describe("getByWeek", () => {
    ...
    test("应支持额外的筛选条件", () => {
      const mockData = [
        createMockRecord({ week_number: 11, insurance_type: "商业险" }),
        createMockRecord({ week_number: 11, insurance_type: "交强险" }),
      ];
