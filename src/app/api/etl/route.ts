...
export async function POST(req: NextRequest) {
  ...
  if (!files || files.length === 0) {
    return NextResponse.json(
      { success: false, message: '没有文件被上传。' },
      { status: 400 }
    )
  }
  if (!outputFileName) {
    return NextResponse.json(
      { success: false, message: '缺少输出文件名。' },
      { status: 400 }
    )
  }
  ...
  if (!sanitizedOutputFileName.endsWith('.duckdb')) {
    return NextResponse.json(
      { success: false, message: '输出文件名必须以 .duckdb 结尾。' },
      { status: 400 }
    )
  }
  ...
  try {
    ...
    try {
      ...
      return NextResponse.json(
        {
          success: false,
          message: `Python 脚本未生成预期的 DuckDB 文件: ${sanitizedOutputFileName}`,
          logs: scriptOutput + scriptError,
        },
        { status: 500 }
      )
