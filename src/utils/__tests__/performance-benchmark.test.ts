...
function generateMockData(count: number): InsuranceRecord[] {
  ...
  for (let i = 0; i < count; i++) {
    data.push({
      ...
      matured_premium: Math.random() * 8000,
      reported_claim_payment: Math.random() * 2000,
      claim_case_count: Math.random() * 1500,
  ...
  describe("缓存Store性能测试", () => {
    it("应该快速设置和获取缓存", () => {
      const mockResult = {
        ...
        policy_count: 500,
        matured_premium: 800000,
        reported_claim_payment: 200000,
        claim_case_count: 150000,
        loss_ratio: 0.25,
      } as any as KPIResult;
  ...
  it("应该显著提升重复查询的性能", () => {
    ...
    const firstQueryDuration = measurePerformance(() => {
      ...
      useCacheStore.getState().setKPICache("test-filter-key", {
        ...
        policy_count: stats.totalPolicyCount,
        matured_premium: 0,
