...
describe("goalCalculator", () => {
  ...
  it("calculates metrics with over achievement", () => {
    ...
    // 缺口 = 目标 - 实际完成，负数表示超额完成
    expect(metrics.initialGap).toBe(-20); // 100 - 120 = -20
    expect(metrics.tunedGap).toBe(-40); // 80 - 120 = -40
    expect(metrics.initialAchievementRate).toBeCloseTo(1.2); // 120/100
    expect(metrics.tunedAchievementRate).toBeCloseTo(1.5); // 120/80
  });
