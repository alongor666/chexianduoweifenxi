...
describe('Real Data Test', () => {
  const realDataPath = '/Users/xuechenglong/Desktop/数据源(家)__chexianduoweifenxi/2025年/2025保单第40周变动成本明细表.csv'

  it('should parse real data correctly', async () => {
    // 检查文件是否存在
    if (!fs.existsSync(realDataPath)) {
      console.warn(`Real data file not found at ${realDataPath}, skipping test.`)
      return
    }
