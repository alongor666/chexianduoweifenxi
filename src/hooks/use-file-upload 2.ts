  ...
  const uploadSingleFile = useCallback(
    async (
      ...
      try {
        ...
        if (isDuckDB) {
          // DuckDB 文件处理
          console.log(`[File Upload] 检测到 DuckDB 文件: ${file.name}`)
          ...
          setProgress({
            ...
            fileName: file.name,
            fileProgress: 50,
            overallProgress: (fileIndex / totalFiles) * 100 + 50 / totalFiles,
          ...
          setProgress({
            ...
            fileName: file.name,
            fileProgress: 80,
            overallProgress: (fileIndex / totalFiles) * 100 + 80 / totalFiles,
          ...
          console.log(`[File Upload] DuckDB 文件加载成功: ${data.length} 条记录`)
          ...
          // CSV 文件处理（原有逻辑）
          console.log(`[File Upload] 检测到 CSV 文件: ${file.name}`)
        ...
        console.error(`[File Upload] 文件处理失败: ${file.name}`, error)
