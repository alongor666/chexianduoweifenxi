...
try {
  ...
  console.log()
  console.log(
    `保单年度 (${stats.years.size}): ${Array.from(stats.years).sort().join(', ')}`
  )
  console.log(
    `周序号 (${stats.weeks.size}): ${Array.from(stats.weeks)
      .sort((a, b) => Number(a) - Number(b))
      .join(', ')}`
  )
  console.log(
    `三级机构 (${stats.organizations.size}): ${Array.from(stats.organizations).sort().join(', ')}`
  )
  console.log(
    `保险类型 (${stats.insuranceTypes.size}): ${Array.from(stats.insuranceTypes).sort().join(', ')}`
  )
  console.log(
    `业务类型 (${stats.businessTypes.size}): ${Array.from(stats.businessTypes).sort().slice(0, 10).join(', ')}...`
  )
  console.log()
