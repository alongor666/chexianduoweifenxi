...
for (let i = 1; i < lines.length; i++) {
  ...
  try {
    ...
    const signedPremium =
      parseFloat(values[fieldIndex['signed_premium_yuan']]) || 0
    const maturedPremium =
      parseFloat(values[fieldIndex['matured_premium_yuan']]) || 0
    ...
    const claimPayment =
      parseFloat(values[fieldIndex['reported_claim_payment_yuan']]) || 0
    const expenseAmount =
      parseFloat(values[fieldIndex['expense_amount_yuan']]) || 0
    const commercialBeforeDiscount =
      parseFloat(
        values[fieldIndex['commercial_premium_before_discount_yuan']]
      ) || 0
    const premiumPlan = parseFloat(values[fieldIndex['premium_plan_yuan']]) || 0
    const marginalContribution =
      parseFloat(values[fieldIndex['marginal_contribution_amount_yuan']]) || 0
...
console.log('-'.repeat(80))
console.log(
  `签单保费总额: ${totalSignedPremium.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} 元`
)
console.log(
  `满期保费总额: ${totalMaturedPremium.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} 元`
)
console.log(`保单总数: ${totalPolicyCount.toLocaleString()} 件`)
console.log(`赔案总数: ${totalClaimCount.toLocaleString()} 件`)
console.log(
  `赔款总额: ${totalReportedClaimPayment.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} 元`
)
console.log(
  `费用总额: ${totalExpenseAmount.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} 元`
)
console.log(
  `商业险折前保费: ${totalCommercialPremiumBeforeDiscount.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} 元`
)
console.log(
  `保费计划: ${totalPremiumPlan.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} 元`
)
console.log(
  `边际贡献额: ${totalMarginalContribution.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} 元`
)
console.log()
...
const contributionMarginRatio =
  totalMaturedPremium !== 0
    ? (totalMarginalContribution / totalMaturedPremium) * 100
    : null
console.log(
  `1. 满期边际贡献率: ${contributionMarginRatio !== null ? contributionMarginRatio.toFixed(2) + '%' : 'N/A'}`
)
console.log(
  `   公式: (${totalMarginalContribution.toFixed(2)} / ${totalMaturedPremium.toFixed(2)}) * 100`
)

// 2. 保费达成率（简化版，假设已过天数/365 = 1）
const premiumAchievementRate =
  totalPremiumPlan !== 0 ? (totalSignedPremium / totalPremiumPlan) * 100 : null
console.log(
  `2. 保费达成率: ${premiumAchievementRate !== null ? premiumAchievementRate.toFixed(2) + '%' : 'N/A'}`
)
console.log(
  `   公式: (${totalSignedPremium.toFixed(2)} / ${totalPremiumPlan.toFixed(2)}) * 100`
)
...
const lossRatio =
  totalMaturedPremium !== 0
    ...
    : null
console.log(
  `3. 满期赔付率: ${lossRatio !== null ? lossRatio.toFixed(2) + '%' : 'N/A'}`
)
console.log(
  `   公式: (${totalReportedClaimPayment.toFixed(2)} / ${totalMaturedPremium.toFixed(2)}) * 100`
)
...
const expenseRatio =
  totalSignedPremium !== 0
    ...
    : null
console.log(
  `4. 费用率: ${expenseRatio !== null ? expenseRatio.toFixed(2) + '%' : 'N/A'}`
)
console.log(
  `   公式: (${totalExpenseAmount.toFixed(2)} / ${totalSignedPremium.toFixed(2)}) * 100`
)
...
const maturityRatio =
  totalSignedPremium !== 0
    ...
    : null
console.log(
  `5. 满期率: ${maturityRatio !== null ? maturityRatio.toFixed(2) + '%' : 'N/A'}`
)
console.log(
  `   公式: (${totalMaturedPremium.toFixed(2)} / ${totalSignedPremium.toFixed(2)}) * 100`
)
...
const claimFrequency =
  totalPolicyCount !== 0
    ...
    : null
console.log(
  `6. 满期出险率: ${claimFrequency !== null ? claimFrequency.toFixed(2) + '%' : 'N/A'}`
)
console.log(
  `   公式: (${totalClaimCount} / ${totalPolicyCount}) * ${maturityRatio?.toFixed(2)}% * 100`
)
...
// 8. 商业险自主系数
const autonomyCoefficient =
  totalCommercialPremiumBeforeDiscount !== 0
    ? totalSignedPremium / totalCommercialPremiumBeforeDiscount
    : null
console.log(
  `8. 商业险自主系数: ${autonomyCoefficient !== null ? autonomyCoefficient.toFixed(4) : 'N/A'}`
)
console.log(
  `   公式: ${totalSignedPremium.toFixed(2)} / ${totalCommercialPremiumBeforeDiscount.toFixed(2)}`
)
console.log()
...
const averagePremium =
  totalPolicyCount !== 0
    ? Math.round(totalSignedPremium / totalPolicyCount)
    : null
const averageClaim =
  totalClaimCount !== 0
    ? Math.round(totalReportedClaimPayment / totalClaimCount)
    : null
const averageExpense =
  totalPolicyCount !== 0
    ? Math.round(totalExpenseAmount / totalPolicyCount)
    : null
...
console.log(
  `4. 单均保费: ${averagePremium !== null ? averagePremium.toLocaleString() + ' 元' : 'N/A'}`
)
console.log(
  `   公式: ROUND(${totalSignedPremium.toFixed(2)} / ${totalPolicyCount})`
)

console.log(
  `5. 案均赔款: ${averageClaim !== null ? averageClaim.toLocaleString() + ' 元' : 'N/A'}`
)
console.log(
  `   公式: ROUND(${totalReportedClaimPayment.toFixed(2)} / ${totalClaimCount})`
)

console.log(
  `6. 单均费用: ${averageExpense !== null ? averageExpense.toLocaleString() + ' 元' : 'N/A'}`
)
console.log(
  `   公式: ROUND(${totalExpenseAmount.toFixed(2)} / ${totalPolicyCount})`
)
console.log()
...
function safeDivide(numerator, denominator, description) {
  ...
  console.log(
    `${description}: ${result !== null ? result.toFixed(4) : 'null (除零保护)'}`
  )
  return result
}
