  ...
  test('应正确加载和过滤数据', async () => {
    ...
    const mockData = [
      ...
      },
      {
        ...
        policy_start_year: 2024,
        week_number: 2,
        business_type_category: '10吨以上营业货车（普货）',
        signed_premium_yuan: 200000,
        third_level_organization: '测试机构',
        customer_category_3: '营业货车',
        insurance_type: '交强险',
      },
      {
        id: '3',
...
describe('新架构 - 集成测试', () => {
  test('应支持完整的数据流程', async () => {
    ...
    const mockData = Array.from({ length: 100 }, (_, i) => ({
      ...
      policy_count: i + 1,
      third_level_organization: '测试机构',
      customer_category_3: i % 2 === 0 ? '非营业个人客车' : '营业货车',
      insurance_type: '交强险',
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    })) as unknown as any[]
