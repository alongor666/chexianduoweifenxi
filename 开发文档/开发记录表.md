...
- **右Y轴(折线图)**:
  - 折线A: 签单保费占比(%)
  - 折线B: 可切换指标(默认变动成本率,可切换为边际贡献率/满期赔付率/费用率)
...
1. **新建图表组件** (`src/components/features/business-type-dual-axis-chart.tsx`)
...
2. **集成到专题分析模块** (`src/components/features/thematic-analysis.tsx`)
...
- `src/components/features/business-type-dual-axis-chart.tsx` (新建)
- `src/components/features/thematic-analysis.tsx` (修改)
- `开发文档/开发记录表.md` (本条目)
