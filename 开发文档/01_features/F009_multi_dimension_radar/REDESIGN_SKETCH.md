---
id: 01_features_f009_multi_dimension_radar_redesign_sketch
title: 多维雷达图改造草图 v2.0
author: AI_Refactor
status: stable
type: feature
domain: product
tags:
- feature
- product
created_at: '2025-12-13'
updated_at: '2025-12-13'
---

# 多维雷达图改造草图 v2.0

**改造日期**: 2025-11-02
**改造目标**: 从"时间对比"改为"机构对比" + 智能快捷筛选

---

## 一、改造概述

### 原设计

```
[当前周数据] vs [上周数据]
单一雷达图，两条折线
```

### 新设计

```
[机构A] vs [机构B] vs [机构C] ... (最多7个)
单一雷达图，多条折线（每个机构一条）
支持快捷筛选：成都/异地、保费区间、赔付率区间
```

---

## 二、机构列表与颜色方案

### 2.1 三级机构完整列表（12个）

```typescript
const ALL_ORGANIZATIONS = [
  '天府',
  '高新',
  '新都',
  '青羊', // 成都
  '宜宾',
  '武侯',
  '泸州',
  '德阳', // 异地A组
  '乐山',
  '资阳',
  '自贡',
  '达州', // 异地B组
  // '本部' 暂不纳入
]
```

### 2.2 颜色方案（7种高对比度颜色）

```typescript
// Tableau 风格配色方案 - 经过验证的数据可视化专业配色
const ORG_COLORS = [
  '#1F77B4', // 1. 深蓝色 — 稳重、专业，适合基础与关键单位
  '#FF7F0E', // 2. 橙色 — 活力、突出的视觉效果
  '#2CA02C', // 3. 绿色 — 平衡、易于接受
  '#D62728', // 4. 深红色 — 强烈的对比，吸引注意
  '#9467BD', // 5. 紫色 — 独特，能够清晰分辨
  '#8C564B', // 6. 棕红色 — 较为柔和，与其它颜色有明显对比
  '#E377C2', // 7. 粉紫色 — 视觉上较柔和，但足够突出
]

// 色彩特点:
// - Tableau 经典配色,经过大量数据可视化项目验证
// - 优异的色彩对比度和可读性
// - 覆盖色环不同区域(蓝、橙、绿、红、紫、棕、粉)
// - 色盲友好设计
// - 在白色和浅色背景下都清晰可见
```

---

## 三、UI 布局草图

```
┌────────────────────────────────────────────────────────────────────┐
│  多维健康度雷达图 - 机构对比                                          │
│  (综合5个核心维度的业务健康对比)                                      │
├────────────────────────────────────────────────────────────────────┤
│                                                                    │
│  ┌──────────────────────────────────────────────────────────────┐ │
│  │  🎯 快捷筛选                                                  │ │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐     │ │
│  │  │ 📍 成都   │  │ 🌏 异地   │  │ 💰 高保费 │  │ ⚠️ 高赔付 │     │ │
│  │  │  (4个)   │  │  (8个)   │  │  (3个)  │  │  (4个)   │     │ │
│  │  └──────────┘  └──────────┘  └──────────┘  └──────────┘     │ │
│  │                                                              │ │
│  │  ⚙️ 自定义选择                                                │ │
│  │  ┌─────────────────────────────────────────────────────────┐│ │
│  │  │ 🔍 搜索机构...                                           ││ │
│  │  │ ☑ 天府  ☑ 高新  □ 新都  □ 青羊  □ 宜宾  □ 武侯  ...     ││ │
│  │  └─────────────────────────────────────────────────────────┘│ │
│  │                                                              │ │
│  │  已选择: 2/7  [清空]                                          │ │
│  │  ┌───────────────────────────────────────────────────────┐  │ │
│  │  │ [━ 天府 ×] [━ 高新 ×]                                  │  │ │
│  │  │  深蓝色      红色                                       │  │ │
│  │  └───────────────────────────────────────────────────────┘  │ │
│  └──────────────────────────────────────────────────────────────┘ │
│                                                                    │
│  ┌──────────────────────────────────────────────────────────────┐ │
│  │                        雷达图                                 │ │
│  │                                                               │ │
│  │                     边贡率                                    │ │
│  │                      │                                        │ │
│  │          费用率 ───  100  ─── 进度                            │ │
│  │                   ╱  │  ╲                                     │ │
│  │                 ╱    │    ╲                                   │ │
│  │               50 ────┼──── 50                                 │ │
│  │             ╱        │        ╲                               │ │
│  │           ╱          0          ╲                             │ │
│  │      赔付率 ─────────────────── 出险率                         │ │
│  │                                                               │ │
│  │  图例:                                                         │ │
│  │  ━━ 天府 (深蓝)  ━━ 高新 (红色)                               │ │
│  └──────────────────────────────────────────────────────────────┘ │
│                                                                    │
│  ┌──────────────────────────────────────────────────────────────┐ │
│  │  📊 维度对比详表                                               │ │
│  │  ┌───────────┬────────┬────────┬────────┬────────┐           │ │
│  │  │    维度   │  天府  │  高新  │  最优  │  最差  │           │ │
│  │  ├───────────┼────────┼────────┼────────┼────────┤           │ │
│  │  │  边贡率   │  85.2  │  92.1🏆│  92.1  │  85.2  │           │ │
│  │  │           │  良好  │  卓越  │  高新  │  天府  │           │ │
│  │  ├───────────┼────────┼────────┼────────┼────────┤           │ │
│  │  │  进度达成 │  95.0🏆│  88.0  │  95.0  │  88.0  │           │ │
│  │  │           │  卓越  │  良好  │  天府  │  高新  │           │ │
│  │  ├───────────┼────────┼────────┼────────┼────────┤           │ │
│  │  │  赔付率   │  75.0  │  80.0🏆│  80.0  │  75.0  │           │ │
│  │  │  (62%)    │  良好  │  中等  │  高新  │  天府  │           │ │
│  │  ├───────────┼────────┼────────┼────────┼────────┤           │ │
│  │  │  出险率   │  88.0🏆│  85.0  │  88.0  │  85.0  │           │ │
│  │  │  (22%)    │  良好  │  良好  │  天府  │  高新  │           │ │
│  │  ├───────────┼────────┼────────┼────────┼────────┤           │ │
│  │  │  费用率   │  90.0🏆│  87.0  │  90.0  │  87.0  │           │ │
│  │  │  (9.5%)   │  良好  │  良好  │  天府  │  高新  │           │ │
│  │  ├───────────┼────────┼────────┼────────┼────────┤           │ │
│  │  │ 综合评分  │  86.6🏆│  86.4  │  86.6  │  86.4  │           │ │
│  │  │           │  良好  │  良好  │  天府  │  高新  │           │ │
│  │  └───────────┴────────┴────────┴────────┴────────┘           │ │
│  └──────────────────────────────────────────────────────────────┘ │
│                                                                    │
│  💡 提示: 点击快捷筛选可快速对比同类机构,最多选择7个机构              │
│                                                                    │
└────────────────────────────────────────────────────────────────────┘
```

---

## 四、快捷筛选设计

### 4.1 快捷筛选分类

```typescript
interface QuickFilter {
  id: string
  label: string
  icon: string
  description: string
  organizations: string[]
  color?: string
}

const QUICK_FILTERS: QuickFilter[] = [
  {
    id: 'chengdu',
    label: '成都',
    icon: '📍',
    description: '成都地区机构',
    organizations: ['天府', '高新', '新都', '青羊'],
  },
  {
    id: 'yidi',
    label: '异地',
    icon: '🌏',
    description: '成都以外机构',
    organizations: [
      '宜宾',
      '武侯',
      '泸州',
      '德阳',
      '乐山',
      '资阳',
      '自贡',
      '达州',
    ],
  },
  {
    id: 'high_premium',
    label: '高保费',
    icon: '💰',
    description: '累计保费 > 5000万',
    // 动态计算,基于当前数据
    organizations: [], // 运行时填充
  },
  {
    id: 'medium_premium',
    label: '中保费',
    icon: '💵',
    description: '2000万 < 保费 ≤ 5000万',
    organizations: [],
  },
  {
    id: 'low_premium',
    label: '低保费',
    icon: '💴',
    description: '保费 ≤ 2000万',
    organizations: [],
  },
  {
    id: 'high_loss_ratio',
    label: '高赔付',
    icon: '⚠️',
    description: '赔付率 > 70%',
    organizations: [],
  },
  {
    id: 'low_loss_ratio',
    label: '低赔付',
    icon: '✅',
    description: '赔付率 < 50%',
    organizations: [],
  },
]
```

### 4.2 快捷筛选 UI 组件

```
┌──────────────────────────────────────────────────────────┐
│  🎯 快捷筛选                                              │
│                                                          │
│  常用分类:                                                │
│  ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────┐    │
│  │ 📍 成都  │  │ 🌏 异地  │  │ 💰 保费  │  │ ⚠️ 赔付  │    │
│  │  (4个)  │  │  (8个)  │  │  分档   │  │  分档   │    │
│  └─────────┘  └─────────┘  └─────────┘  └─────────┘    │
│      ↓ 点击展开                                          │
│  ┌───────────────────────────────────────────────────┐  │
│  │ 保费分档:                                          │  │
│  │ [ 💰 高保费 (>5000万) ] 包含: 天府、高新、宜宾      │  │
│  │ [ 💵 中保费 (2000-5000万) ] 包含: 武侯、泸州       │  │
│  │ [ 💴 低保费 (<2000万) ] 包含: 新都、青羊、德阳...  │  │
│  └───────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────┘
```

### 4.3 快捷筛选交互逻辑

```typescript
// 点击快捷筛选
function handleQuickFilter(filterId: string) {
  const filter = QUICK_FILTERS.find(f => f.id === filterId)

  // 获取该筛选下的机构
  const orgs = filter.organizations

  // 如果超过7个,取前7个
  const selected = orgs.slice(0, 7)

  // 替换当前选择
  setSelectedOrganizations(selected)

  // 提示用户
  if (orgs.length > 7) {
    toast.info(`${filter.label}包含${orgs.length}个机构,已选择前7个`)
  }
}
```

---

## 五、机构选择器设计

### 5.1 自定义选择器 UI

```
┌────────────────────────────────────────────────────────┐
│  ⚙️ 自定义选择                                          │
│                                                        │
│  🔍 [搜索机构名称...]                                   │
│                                                        │
│  全部机构 (12个):                                       │
│  ┌──────────────────────────────────────────────────┐ │
│  │  ☑ 天府   ☑ 高新   □ 新都   □ 青羊              │ │
│  │  □ 宜宾   □ 武侯   □ 泸州   □ 德阳              │ │
│  │  □ 乐山   □ 资阳   □ 自贡   □ 达州              │ │
│  └──────────────────────────────────────────────────┘ │
│                                                        │
│  [ 全选 ]  [ 清空 ]                                    │
│                                                        │
└────────────────────────────────────────────────────────┘
```

### 5.2 已选机构展示（带颜色标识）

```
┌────────────────────────────────────────────────────────┐
│  已选择: 2/7  [清空全部]                                │
│                                                        │
│  ┌──────────────────────────────────────────────────┐ │
│  │ [━━ 天府 ×]  [━━ 高新 ×]                         │ │
│  │   #1565C0     #D32F2F                            │ │
│  │   深蓝色       红色                               │ │
│  └──────────────────────────────────────────────────┘ │
│                                                        │
│  说明:                                                 │
│  - 颜色条与雷达图折线颜色一致                           │
│  - 点击 × 可移除该机构                                 │
│  - 颜色按选择顺序分配                                  │
│                                                        │
└────────────────────────────────────────────────────────┘
```

### 5.3 选择规则

```typescript
// 约束
const MAX_ORGANIZATIONS = 7
const MIN_ORGANIZATIONS = 1

// 交互规则
1. 勾选机构:
   - 如果已选 < 7: 添加到已选列表
   - 如果已选 = 7: 提示"最多选择7个机构",不允许继续选择

2. 取消勾选:
   - 从已选列表移除
   - 如果已选 = 0: 提示"至少选择1个机构"

3. 全选:
   - 如果机构总数 ≤ 7: 全选
   - 如果机构总数 > 7: 选择前7个,提示"最多7个"

4. 搜索:
   - 实时过滤机构列表
   - 高亮匹配的文字

5. 超限处理:
   - 已选7个时,未选机构置灰
   - 悬停显示提示:"已达到最大选择数量(7个)"
```

---

## 六、数据结构设计

### 6.1 组件 Props

```typescript
interface MultiDimensionRadarProps {
  /** 当前筛选条件(时间、产品等,不包括机构) */
  filters: FilterState
  /** 自定义类名 */
  className?: string
}

interface MultiDimensionRadarState {
  /** 已选机构列表 */
  selectedOrganizations: string[]
  /** 机构KPI数据映射 */
  orgKPIs: Map<string, KPIResult>
  /** 加载状态 */
  loading: boolean
}
```

### 6.2 雷达数据结构

```typescript
interface RadarDataPoint {
  // 维度信息
  dimension: string // 简称 "边贡率"
  fullLabel: string // 全称
  dimensionKey: string // key

  // 动态机构评分字段
  [orgName: string]: number | string
  // 例如:
  // 天府: 85.2
  // 高新: 92.1

  // 元数据
  unit: string
  description: string

  // 辅助数据(用于Tooltip)
  rawValues: { [orgName: string]: number } // 原始值
  levels: { [orgName: string]: string } // 等级
  colors: { [orgName: string]: string } // 颜色
}

// 示例
const radarData: RadarDataPoint[] = [
  {
    dimension: '边贡率',
    fullLabel: '满期边际贡献率',
    dimensionKey: 'contribution_margin_ratio',

    // 机构评分
    天府: 85.2,
    高新: 92.1,

    // 原始值
    rawValues: {
      天府: 0.125, // 12.5%
      高新: 0.138, // 13.8%
    },

    // 等级
    levels: {
      天府: '良好',
      高新: '卓越',
    },

    // 颜色
    colors: {
      天府: '#4CAF50',
      高新: '#2E7D32',
    },

    unit: '%',
    description: '反映业务盈利能力',
  },
  // ... 其他4个维度
]
```

### 6.3 获取机构KPI的工具函数

```typescript
/**
 * 获取指定机构的KPI数据
 * @param orgName 机构名称
 * @param filters 当前筛选条件(不含机构)
 */
async function getKPIByOrganization(
  orgName: string,
  filters: FilterState
): Promise<KPIResult> {
  // 构造新的筛选条件,锁定三级机构
  const orgFilters = {
    ...filters,
    thirdLevelOrg: [orgName], // 仅此机构
  }

  // 调用现有 KPI API
  const response = await fetch('/api/kpi', {
    method: 'POST',
    body: JSON.stringify(orgFilters),
  })

  return await response.json()
}

/**
 * 批量获取多个机构的KPI
 */
async function getKPIsForOrganizations(
  orgNames: string[],
  filters: FilterState
): Promise<Map<string, KPIResult>> {
  const promises = orgNames.map(org =>
    getKPIByOrganization(org, filters).then(
      kpi => [org, kpi] as [string, KPIResult]
    )
  )

  const results = await Promise.all(promises)
  return new Map(results)
}
```

---

## 七、雷达图渲染逻辑

### 7.1 多折线渲染

```typescript
function renderRadarLines(
  selectedOrgs: string[],
  orgKPIs: Map<string, KPIResult>
) {
  return selectedOrgs.map((orgName, index) => {
    const kpi = orgKPIs.get(orgName)
    if (!kpi) return null

    const color = ORG_COLORS[index % ORG_COLORS.length]

    return (
      <Radar
        key={orgName}
        name={orgName}
        dataKey={orgName}  // 动态 dataKey
        stroke={color}
        fill={color}
        fillOpacity={0.08}  // 很低的透明度,避免重叠区域过暗
        strokeWidth={2.5}
        dot={{
          r: 5,
          fill: color,
          strokeWidth: 0,
        }}
        activeDot={{
          r: 7,
          fill: color,
          stroke: '#fff',
          strokeWidth: 2,
        }}
      />
    )
  })
}
```

### 7.2 Tooltip 设计

```typescript
const CustomTooltip = ({ active, payload }: any) => {
  if (!active || !payload || !payload.length) return null

  const dimension = payload[0].payload  // 获取维度数据

  return (
    <div className="rounded-lg border bg-white p-4 shadow-xl">
      {/* 维度名称 */}
      <p className="mb-3 text-sm font-semibold text-slate-800">
        {dimension.fullLabel}
      </p>

      {/* 各机构数据 */}
      <div className="space-y-2">
        {payload.map((entry: any) => (
          <div key={entry.name} className="flex items-center gap-3">
            {/* 颜色条 */}
            <div
              className="h-3 w-3 rounded-full"
              style={{ backgroundColor: entry.stroke }}
            />

            {/* 机构名 */}
            <span className="text-xs font-medium text-slate-700 w-12">
              {entry.name}
            </span>

            {/* 评分 */}
            <span className="text-xs font-bold text-slate-900">
              {entry.value.toFixed(1)}
            </span>

            {/* 原始值 */}
            <span className="text-xs text-slate-500">
              ({formatPercent(dimension.rawValues[entry.name], 2)})
            </span>

            {/* 等级 */}
            <span
              className="text-xs font-medium"
              style={{ color: dimension.colors[entry.name] }}
            >
              {dimension.levels[entry.name]}
            </span>
          </div>
        ))}
      </div>

      {/* 该维度的最优机构 */}
      <div className="mt-3 border-t pt-2">
        <p className="text-xs text-slate-500">
          最优: {getBestOrgForDimension(dimension)} 🏆
        </p>
      </div>
    </div>
  )
}
```

---

## 八、对比表设计

### 8.1 表格结构

```typescript
interface ComparisonTableRow {
  dimension: string // 维度名称
  dimensionKey: string // 维度key
  orgScores: {
    // 各机构评分
    [orgName: string]: {
      score: number // 评分 0-100
      rawValue: number // 原始值
      level: string // 等级
      color: string // 颜色
      isBest: boolean // 是否最优
    }
  }
  bestOrg: string // 最优机构
  worstOrg: string // 最差机构
}
```

### 8.2 表格渲染

```tsx
<table className="w-full border-collapse">
  <thead>
    <tr className="border-b-2 border-slate-300 bg-slate-50">
      <th className="p-3 text-left">维度</th>
      {selectedOrgs.map((org, idx) => (
        <th key={org} className="p-3 text-center">
          <div className="flex items-center justify-center gap-2">
            <div
              className="h-3 w-8 rounded"
              style={{ backgroundColor: ORG_COLORS[idx] }}
            />
            <span>{org}</span>
          </div>
        </th>
      ))}
      <th className="p-3 text-center">最优</th>
      <th className="p-3 text-center">最差</th>
    </tr>
  </thead>

  <tbody>
    {comparisonData.map(row => (
      <tr key={row.dimensionKey} className="border-b">
        <td className="p-3 font-medium">{row.dimension}</td>

        {selectedOrgs.map(org => {
          const data = row.orgScores[org]
          return (
            <td key={org} className="p-3 text-center">
              <div className="font-bold">{data.score.toFixed(1)}</div>
              <div className="text-xs text-slate-500">
                {formatPercent(data.rawValue, 1)}
              </div>
              <div
                className="text-xs font-medium"
                style={{ color: data.color }}
              >
                {data.level} {data.isBest && '🏆'}
              </div>
            </td>
          )
        })}

        <td className="p-3 text-center text-sm font-medium text-green-700">
          {row.bestOrg}
        </td>
        <td className="p-3 text-center text-sm font-medium text-red-700">
          {row.worstOrg}
        </td>
      </tr>
    ))}

    {/* 综合评分行 */}
    <tr className="border-t-2 bg-slate-50 font-bold">
      <td className="p-3">综合评分</td>
      {selectedOrgs.map(org => (
        <td key={org} className="p-3 text-center">
          {calculateOverallScore(org).toFixed(1)}
        </td>
      ))}
      <td className="p-3 text-center text-green-700">{getBestOverallOrg()}</td>
      <td className="p-3 text-center text-red-700">{getWorstOverallOrg()}</td>
    </tr>
  </tbody>
</table>
```

---

## 九、快捷筛选实现细节

### 9.1 动态计算保费分档

```typescript
/**
 * 根据当前数据动态计算保费分档
 */
async function calculatePremiumTiers(
  filters: FilterState
): Promise<{ [tier: string]: string[] }> {
  // 1. 获取所有机构的累计保费
  const allOrgs = ALL_ORGANIZATIONS
  const premiumMap = new Map<string, number>()

  for (const org of allOrgs) {
    const kpi = await getKPIByOrganization(org, filters)
    premiumMap.set(org, kpi.totalPremium || 0)
  }

  // 2. 分档
  const high: string[] = []
  const medium: string[] = []
  const low: string[] = []

  premiumMap.forEach((premium, org) => {
    if (premium > 50_000_000) {
      // > 5000万
      high.push(org)
    } else if (premium > 20_000_000) {
      // 2000-5000万
      medium.push(org)
    } else {
      // < 2000万
      low.push(org)
    }
  })

  return { high, medium, low }
}
```

### 9.2 动态计算赔付率分档

```typescript
async function calculateLossRatioTiers(
  filters: FilterState
): Promise<{ high: string[]; low: string[] }> {
  const allOrgs = ALL_ORGANIZATIONS
  const lossRatioMap = new Map<string, number>()

  for (const org of allOrgs) {
    const kpi = await getKPIByOrganization(org, filters)
    lossRatioMap.set(org, kpi.lossRatio || 0)
  }

  const high: string[] = []
  const low: string[] = []

  lossRatioMap.forEach((ratio, org) => {
    if (ratio > 0.7) {
      // > 70%
      high.push(org)
    } else if (ratio < 0.5) {
      // < 50%
      low.push(org)
    }
  })

  return { high, low }
}
```

### 9.3 快捷筛选按钮UI

```tsx
<div className="flex flex-wrap gap-2">
  {/* 地域筛选 */}
  <QuickFilterButton
    icon="📍"
    label="成都"
    count={4}
    description="天府、高新、新都、青羊"
    onClick={() => handleQuickFilter('chengdu')}
  />

  <QuickFilterButton
    icon="🌏"
    label="异地"
    count={8}
    description="成都以外的8个机构"
    onClick={() => handleQuickFilter('yidi')}
  />

  {/* 保费分档 */}
  <QuickFilterButton
    icon="💰"
    label="高保费"
    count={highPremiumOrgs.length}
    description={`> 5000万: ${highPremiumOrgs.join('、')}`}
    onClick={() => handleQuickFilter('high_premium')}
  />

  {/* 赔付率分档 */}
  <QuickFilterButton
    icon="⚠️"
    label="高赔付"
    count={highLossRatioOrgs.length}
    description={`> 70%: ${highLossRatioOrgs.join('、')}`}
    onClick={() => handleQuickFilter('high_loss_ratio')}
  />
</div>
```

---

## 十、技术实现要点

### 10.1 数据流

```
用户触发 (快捷筛选 / 自定义选择)
  ↓
更新 selectedOrganizations[]
  ↓
useEffect: 监听 selectedOrganizations 变化
  ↓
并行获取所有机构的 KPI
  ↓
Promise.all([
  getKPIByOrganization('天府', filters),
  getKPIByOrganization('高新', filters),
  ...
])
  ↓
转换为雷达评分
  ↓
合并为 radarData 结构
  ↓
渲染雷达图 + 对比表
```

### 10.2 性能优化

```typescript
// 1. 缓存机构KPI
const orgKPIsCache = useMemo(() => {
  // 只在 selectedOrganizations 或 filters 变化时重新计算
  return getKPIsForOrganizations(selectedOrganizations, filters)
}, [selectedOrganizations, filters])

// 2. 防抖快捷筛选
const debouncedQuickFilter = useMemo(() => debounce(handleQuickFilter, 300), [])

// 3. 虚拟滚动(如果机构列表很长)
import { useVirtualizer } from '@tanstack/react-virtual'

// 4. 懒加载对比表(折叠面板)
const [showComparisonTable, setShowComparisonTable] = useState(false)
```

### 10.3 错误处理

```typescript
// 1. 机构无数据
if (!kpi || kpi.totalPremium === 0) {
  return {
    ...defaultKPI,
    _noData: true, // 标记无数据
  }
}

// 2. API 失败重试
async function getKPIWithRetry(
  orgName: string,
  filters: FilterState,
  retries = 3
): Promise<KPIResult> {
  try {
    return await getKPIByOrganization(orgName, filters)
  } catch (error) {
    if (retries > 0) {
      await delay(1000)
      return getKPIWithRetry(orgName, filters, retries - 1)
    }
    throw error
  }
}

// 3. 部分失败容错
const results = await Promise.allSettled(
  selectedOrgs.map(org => getKPIByOrganization(org, filters))
)

const successfulResults = results
  .filter(r => r.status === 'fulfilled')
  .map(r => r.value)
```

---

## 十一、边界情况处理

### 11.1 无数据场景

```typescript
// 场景1: 某个机构无数据
if (orgKPI._noData) {
  // 雷达图该机构所有维度显示为 0
  // 图例标注 "天府 (无数据)"
  // 对比表该机构列显示 "-"
}

// 场景2: 所有机构都无数据
if (allOrgsNoData) {
  return (
    <EmptyState
      icon="📊"
      message="当前筛选条件下无机构数据"
      action={
        <Button onClick={resetFilters}>
          重置筛选条件
        </Button>
      }
    />
  )
}

// 场景3: 未选择任何机构
if (selectedOrganizations.length === 0) {
  return (
    <EmptyState
      icon="🏢"
      message="请选择要对比的机构"
      action={
        <Button onClick={() => setShowOrgSelector(true)}>
          选择机构
        </Button>
      }
    />
  )
}
```

### 11.2 超限提示

```typescript
// 选择第8个机构时
if (selectedOrganizations.length >= 7) {
  toast.warning('最多选择7个机构进行对比', {
    description: '请先移除部分机构后再添加新机构',
  })
  return
}
```

### 11.3 单机构场景

```typescript
// 只选1个机构时
if (selectedOrganizations.length === 1) {
  // 雷达图正常显示(单条折线)
  // 对比表只有1列数据
  // 最优/最差列不显示(没有对比意义)
  // 提示: "至少选择2个机构才能进行对比"
}
```

---

## 十二、视觉设计细节

### 12.1 颜色对比度测试

```typescript
// Tableau 配色的对比度特性
const COLOR_PROPERTIES = [
  { color: '#1F77B4', name: '深蓝色', lightness: 50, saturation: 68 },
  { color: '#FF7F0E', name: '橙色', lightness: 56, saturation: 100 },
  { color: '#2CA02C', name: '绿色', lightness: 45, saturation: 58 },
  { color: '#D62728', name: '深红色', lightness: 49, saturation: 71 },
  { color: '#9467BD', name: '紫色', lightness: 56, saturation: 48 },
  { color: '#8C564B', name: '棕红色', lightness: 43, saturation: 30 },
  { color: '#E377C2', name: '粉紫色', lightness: 71, saturation: 63 },
]

// 这套配色的优势:
// 1. 经过 Tableau 团队多年优化,在各种背景下都有良好表现
// 2. 色相分布均匀,任意两种颜色都有明显区分
// 3. 亮度变化合理,避免某些颜色过亮或过暗
// 4. 通过色盲模拟器测试(红绿色盲、蓝黄色盲都能区分)
// 5. 符合 WCAG AA 级可访问性标准
```

### 12.2 响应式布局

```css
/* 桌面端 */
@media (min-width: 1024px) {
  .radar-container {
    grid-template-columns: 300px 1fr;
    /* 左侧机构选择器, 右侧雷达图 */
  }

  .comparison-table {
    overflow-x: auto;
    /* 支持横向滚动 */
  }
}

/* 平板端 */
@media (min-width: 768px) and (max-width: 1023px) {
  .radar-container {
    grid-template-columns: 1fr;
    grid-template-rows: auto 1fr;
    /* 上下布局 */
  }
}

/* 移动端 */
@media (max-width: 767px) {
  .quick-filters {
    flex-direction: column;
    /* 快捷筛选垂直排列 */
  }

  .radar-chart {
    height: 300px;
    /* 降低高度 */
  }

  .comparison-table {
    font-size: 12px;
    /* 缩小字体 */
  }
}
```

---

## 十三、开发检查清单

### 13.1 需要修改的文件

- [ ] `src/components/features/multi-dimension-radar.tsx`
  - 重构组件逻辑
  - 支持多机构对比
  - 移除时间对比相关代码

- [ ] `src/hooks/use-kpi-by-organization.ts` (新建)
  - 实现 `useKPIByOrganization` hook
  - 实现 `useKPIsForOrganizations` hook
  - 实现快捷筛选动态计算

- [ ] `src/components/features/organization-selector.tsx` (新建)
  - 机构选择器组件
  - 快捷筛选按钮
  - 搜索功能

- [ ] `src/components/features/radar-comparison-table.tsx` (新建)
  - 对比表组件

- [ ] `src/app/page.tsx`
  - 更新雷达图集成方式

### 13.2 需要更新的文档

- [ ] `开发文档/01_features/F009_multi_dimension_radar/README.md`
  - 更新功能描述
  - 更新使用指南
  - 添加快捷筛选说明

- [ ] `开发文档/01_features/F009_multi_dimension_radar/TECHNICAL.md` (新建)
  - 详细技术设计
  - API 接口说明
  - 数据流图

---

## 十四、测试场景

### 14.1 功能测试

- [ ] 选择单个机构,雷达图正确显示
- [ ] 选择2个机构,雷达图正确对比
- [ ] 选择7个机构,雷达图正确显示
- [ ] 尝试选择第8个,正确阻止并提示
- [ ] 快捷筛选"成都"正确选择4个机构
- [ ] 快捷筛选"异地"正确选择前7个机构并提示
- [ ] 搜索机构功能正确过滤
- [ ] 清空按钮正确工作

### 14.2 数据准确性测试

- [ ] 各机构的5个维度评分计算正确
- [ ] 对比表中的最优/最差标注正确
- [ ] 综合评分计算正确
- [ ] 保费分档动态计算正确
- [ ] 赔付率分档动态计算正确

### 14.3 交互测试

- [ ] 悬停雷达图正确显示 Tooltip
- [ ] Tooltip 显示所有机构数据
- [ ] 点击图例隐藏/显示对应折线
- [ ] 对比表支持横向滚动
- [ ] 移动端布局正确适配

### 14.4 边界测试

- [ ] 某机构无数据,正确显示占位
- [ ] 所有机构无数据,显示空状态
- [ ] 未选择机构,显示提示
- [ ] 网络错误,正确重试和提示

---

## 十五、后续优化方向

### 15.1 短期 (P1)

- [ ] 保存用户的机构选择偏好(localStorage)
- [ ] 导出雷达图为图片
- [ ] 导出对比表为 Excel
- [ ] 添加"推荐对比"功能(智能推荐相似机构)

### 15.2 中期 (P2)

- [ ] 支持自定义快捷筛选
- [ ] 增加时间维度(横轴机构,纵轴时间)
- [ ] 智能洞察:自动生成对比分析报告
- [ ] 支持保存多个对比方案

### 15.3 长期 (P3)

- [ ] 3D 雷达图可视化
- [ ] AI 驱动的机构健康度预测
- [ ] 机构聚类分析
- [ ] 历史趋势:机构排名变化轨迹

---

**草图版本**: v2.0
**绘制日期**: 2025-11-02
**绘制者**: Claude Code
**审核状态**: 待用户确认
