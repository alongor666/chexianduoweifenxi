...
- **强制要求**: 所有柱状图、表格、文字说明必须排序
- **排序方向**: 从最差 → 最好
- **实现方式**: 使用 `src/config/thresholds.ts` 中的 `sortByValue()` 函数
- **详细规范**: 参见 [核心指标计算引擎 - 7.1 全局排序规范](../../03_technical_design/core_calculations.md#71-全局排序规范--强制规则)

**具体应用**：
- 正向指标（如时间进度达成率）：由低到高排序，最差的在前
- 逆向指标（如变动成本率、赔付率）：由高到低排序，最差的在前
...
### 4. 周序号筛选模式
- **支持模式**: 驾驶舱支持周序号在单选/多选间灵活切换
- **实现方式**: 通过全局工具栏的紧凑化时间筛选器切换模式
- **单选模式**: 适合深度分析单周数据
- **多选模式**: 适合对比多周趋势，支持全选、反选、清空等批量操作
