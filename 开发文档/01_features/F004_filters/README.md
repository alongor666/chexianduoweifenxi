...
- ✅ **紧凑化时间筛选**: 弹出式面板，年度筛选恒定可多选；周序号会根据当前板块自动切换——KPI看板与专题分析锁定单周，驾驶舱、周趋势分析与多维图表支持在单选/多选间切换，并提供全选、反选、清空等批量操作；进入"周趋势分析"标签时自动切换为多选并默认全选全部可用周次，确保周增量分析即时可用。
...
- **新增**: 驾驶舱页面现在支持周序号在单选/多选间灵活切换
- **改进**: 更新 `resolveWeekSelectionMode` 函数，将 `cockpit` 标签页设置为 `flexible` 模式
- **文档**: 更新筛选器文档和驾驶舱功能文档，说明多选模式的使用场景
