...
> 🛠️ **维护工具**: 本文档配套自动化脚本 `scripts/update_dev_records.py`。
> 运行 `python3 scripts/update_dev_records.py` 可执行以下操作：
> 1. ✅ **链接验证**: 检查文档中引用的代码文件是否存在。
> 2. 📊 **进度统计**: 自动统计已完成和待办任务的数量。
> 3. 🔍 **全库扫描**: 扫描整个开发文档库，发现散落在其他文件中的待办事项。
...
- 更新 `开发文档/README.md` 中的图表库描述、脚本路径与 Issue 记录入口，使其对应当前代码仓库结构。
- 调整 `开发文档/00_conventions.md` 的目录树、脚本命令和问题记录引用，删除已不存在的 `03_logs/` 链接。
- 修正 `开发文档/03_technical_design/tech_stack.md` 的图表依赖与第三方库版本，移除仓库中不存在的依赖。
- 在 `开发文档/文档代码一致性分析报告.md` 顶部添加 Reality Alignment 说明，仅声明失效状态。

### 验证状态

- 已验证（node scripts/analyze-codebase.js 成功运行）
...
## 2025-12-13 企业驾驶舱 (Enterprise Cockpit) 重构

### 变更概述
统一驾驶舱布局与可视化规范，提供业务健康快照与经营观察入口。基于统一的5级阈值体系和"从最差到最好"排序规范，实现核心KPI监控和风险预警。

### 已完成 (Done)
- [x] 重构 KPI Metrics Row 为4个核心指标 ([src/components/features/cockpit/kpi-metrics-row.tsx](../src/components/features/cockpit/kpi-metrics-row.tsx))
- [x] 重构 Statistics Row 为4个风险数量型指标 ([src/components/features/cockpit/statistics-row.tsx](../src/components/features/cockpit/statistics-row.tsx))
- [x] 新增 `useBusinessTypeComparison()` hook ([src/hooks/use-comparison-analysis.ts](../src/hooks/use-comparison-analysis.ts))
- [x] 重构 TimeProgressAnalysis 组件 ([src/components/features/cockpit/business-observation/TimeProgressAnalysis.tsx](../src/components/features/cockpit/business-observation/TimeProgressAnalysis.tsx))
- [x] 重构 CostRiskAnalysis 组件 ([src/components/features/cockpit/business-observation/CostRiskAnalysis.tsx](../src/components/features/cockpit/business-observation/CostRiskAnalysis.tsx))
- [x] 统一5级阈值体系（卓越/优秀/健康/预警/危险）
- [x] 实现所有图表强制从最差到最好排序
- [x] 实现一行一图的统一布局

### 待办/计划中 (Todo)
- [ ] 实现时间多选筛选功能 (短期) [驾驶舱页面目前被强制锁定为单选模式，需放开限制]
- [ ] 添加环比变化值和变化幅度显示 (短期)
- [x] 优化移动端响应式布局 (短期) [已验证: 核心组件支持 grid 响应式]
- [ ] 增强总体趋势图表的交互性 (中期)
- [ ] 添加驾驶舱数据导出功能 (中期)
- [ ] 实现自定义阈值配置 (中期)

### 来源文档
- [开发文档/01_features/F016_enterprise_cockpit/README.md](01_features/F016_enterprise_cockpit/README.md)

---

## 2025-12-12 专题分析模块新增业务类型双Y轴复合图

### 需求背景

需要在趋势分析板块中新增一个双Y轴复合图,用于经营分析工具,帮助用户从规模与效率双维度分析各业务类型的表现。
...
- **右Y轴(折线图)**:
  - 折线A: 签单保费占比(%)
  - 折线B: 可切换指标(默认变动成本率,可切换为边际贡献率/边际贡献率/满期赔付率/费用率)

**设计原则**:

- 完全遵守项目既有设定与治理规则
- 最大化复用既有组件、函数、option构建逻辑与主题配置
- 保持低耦合、易维护的代码架构

### 技术实现

1. **新建图表组件** ([src/components/features/business-type-dual-axis-chart.tsx](../src/components/features/business-type-dual-axis-chart.tsx))
   - 使用ECharts复合图表(bar + line + line)
   - 复用项目标准的图表构建函数(`buildGrid`, `buildTooltip`, `buildLegend`, `buildXAxis`, `buildYAxis`, `buildBarSeries`, `buildLineSeries`)
   - 复用业务类型维度定义(`CANONICAL_BUSINESS_TYPES`, `getBusinessTypeLabel`)
   - 复用格式化函数(`formatNumber`, `formatPercent`)
   - 复用主题配置(`CHART_COLORS`)

2. **集成到专题分析模块** ([src/components/features/thematic-analysis.tsx](../src/components/features/thematic-analysis.tsx))
   - 新增"趋势分析"标签页(`TrendAnalysisTab`)
   - 调整TabsList为3列布局,添加"趋势分析"标签
   - 导入并使用`BusinessTypeDualAxisChart`组件

3. **核心特性**:
   - 按标准业务类型顺序展示,自动过滤无数据项
   - 指标切换仅影响"折线B"的取数、格式与状态颜色
   - 自定义Tooltip展示完整信息(业务类型、保费、占比、指标值)
   - 响应项目筛选条件,实时更新数据

### 遵循的规范

- ✅ 业务类型维度: 使用`CANONICAL_BUSINESS_TYPES`和`getBusinessTypeLabel`
- ✅ 指标定义: 严格遵循`core_calculations.md`中的计算公式
- ✅ 图表主题: 使用`src/lib/charts/theme.ts`的统一配置
- ✅ 格式化: 使用`src/utils/formatters/`的标准函数
- ✅ 构建函数: 使用`src/lib/charts/builders.ts`的复用函数
- ✅ 颜色编码: 遵循项目既有的指标颜色规范

### 修改文件

- [src/components/features/business-type-dual-axis-chart.tsx](../src/components/features/business-type-dual-axis-chart.tsx) (新建)
- [src/components/features/thematic-analysis.tsx](../src/components/features/thematic-analysis.tsx) (修改)
- `开发文档/开发记录表.md` (本条目)

### 测试验证

- ⏳ 等待本地环境安装依赖后验证
- 需验证: 图表正确渲染、指标切换功能、数据筛选联动、Tooltip显示

---

## 2025-12-07 ECharts 统一架构迁移 (Phase 1)

### 变更概述
建立统一的 ECharts 基础组件体系，将所有图表组件从 Recharts 迁移到 ECharts。

### 已完成 (Done)
- [x] 创建 BaseEChart 基础组件
- [x] 创建主题配置系统 (theme.ts)
...
### 待办/下一步 (Todo)
- [x] 迁移 DistributionPieChart [已验证: src/components/features/distribution-pie-chart.tsx 已使用 BaseEChart]
- [ ] 本地验证功能
- [ ] 性能测试

### 来源文档
- [开发文档/04_refactoring/UNIFIED_ECHARTS_ARCHITECTURE.md](04_refactoring/UNIFIED_ECHARTS_ARCHITECTURE.md)

---

## 2025-11-12 DuckDB 集成 (Feature/duckdb-integration)

### 变更概述
从 CSV 解析 + IndexedDB 升级到 DuckDB-WASM，以提升性能。

### 已完成 (Done)
- [x] Git 分支管理与提交
- [x] Python 数据转换工具 (scripts/csv_to_duckdb.py)
- [x] 前端数据库适配器 (src/lib/database/)
- [x] 完整技术文档 (DUCKDB_QUICKSTART.md, README_DUCKDB.md)
- [ ] 前端集成 ([src/components/features/file-upload.tsx](../src/components/features/file-upload.tsx)) [UI已支持选择，但 UploadDataUseCase 尚未实现 DuckDB 解析逻辑]

### 需验证 (To Verify)
- [x] 验证 .duckdb 文件上传功能 [后端验证通过: insurance_data.duckdb 文件有效且包含 31.7万 条记录]
- [ ] 验证性能提升指标 (查询性能 10-20倍) [需基准测试]

### 来源文档
- [开发文档/archive/1112开发记录.md](archive/1112开发记录.md)
...
- [src/app/page.tsx](../src/app/page.tsx): 更新标签校验列表，移除 `forecast` 标签；根据当前标签决定 `viewMode`；向 `TopToolbar` 传递当前标签用于模式解析。
- [src/components/layout/analysis-tabs.tsx](../src/components/layout/analysis-tabs.tsx): 删除预测分析枚举项与按钮。
- [src/components/layout/top-toolbar.tsx](../src/components/layout/top-toolbar.tsx): 新增周选择模式解析逻辑，将模式传递给 `CompactTimeFilter`。
- [src/components/filters/compact-time-filter.tsx](../src/components/filters/compact-time-filter.tsx): 接收模式入参，使用 `useFiltering` 同步两个 Store，并根据模式控制单选/多选切换与批量操作。
- [src/components/features/filter-management-panel.tsx](../src/components/features/filter-management-panel.tsx): 适配新签名，默认启用灵活模式。
...
- [src/components/features/data-management-panel.tsx](../src/components/features/data-management-panel.tsx)
- `开发文档/开发记录表.md`
...
- [src/components/features/data-management-panel.tsx](../src/components/features/data-management-panel.tsx) - 集成 FilterPanel 组件
...
2. **[src/components/features/upload-week-preview.tsx](../src/components/features/upload-week-preview.tsx)** - 周次预览组件
   - 显示待导入文件的周次信息
   - 标注新周次和冲突周次
   - 预估导入记录数
   - 提供确认/取消操作
...
1. **[src/lib/storage/data-persistence.ts](../src/lib/storage/data-persistence.ts)**
   - 添加 `WeekInfo` 类型定义
...
2. **[src/hooks/use-file-upload.ts](../src/hooks/use-file-upload.ts)**
...
1. **package.json** - 内存配置
2. **[src/hooks/use-kpi-trend.ts](../src/hooks/use-kpi-trend.ts)** - 核心算法优化
...
1. **[src/lib/utils/array-utils.ts](../src/lib/utils/array-utils.ts)** - 新增工具函数（3个函数，详细注释）
...
1. **[src/utils/radar-score.ts](../src/utils/radar-score.ts)** - 评分转换工具
   - 定义5个维度的评分转换函数
   - 实现原始值到标准化评分的线性插值映射
   - 提供批量转换接口

2. **[src/components/features/multi-dimension-radar.tsx](../src/components/features/multi-dimension-radar.tsx)** - 雷达图组件
   - 使用 Recharts 库绘制雷达图
   - 实现悬停交互和高亮效果
   - 展示维度详细卡片
   - 支持当前周与上周对比
...
## 2025-10-19 主题分析模块优化 (Thematic Analysis)

### 变更概述
完善 KPI 数据契约，重构核心卡片，调整色彩系统阈值。

### 已完成 (Done)
- [x] 完善 KPI 数据契约 ([src/types/insurance.ts](../src/types/insurance.ts))
- [x] 重构 TimeProgressAnalysisCard (引入 objectiveKpi 参数)
- [x] 调整色彩系统阈值 ([src/utils/color-scale.ts](../src/utils/color-scale.ts))
- [x] 添加边贡分析第4张卡片 (单均边贡额)
- [x] 性能优化 - useMemo 缓存

### 遗留问题 (Issues)
- [x] 修复类型错误 - formatCurrency 参数类型 (src/components/features/compact-kpi-dashboard.tsx [已移除]) [已解决: 组件已移除且工具函数已更新]

### 来源文档
- [开发文档/archive/主题分析模块测试记录.md](archive/主题分析模块测试记录.md)
...
1.  **阶段一：扫描`开发文档/`目录，定位所有含开发记录的文件。** (状态：已完成)
2.  **阶段二：提取、分类所有开发记录，并将其整合至`开发文档/开发记录表.md`。** (状态：已完成)
3.  **阶段三：为`开发记录表.md`中的每项记录建立到源文档和相关代码的索引链接。** (状态：进行中)
4.  **阶段四：创建或更新一个自动化脚本来维护开发记录的同步。** (状态：待处理)
5.  **阶段五：清理源文档中已迁移的开发记录。** (状态：待处理)