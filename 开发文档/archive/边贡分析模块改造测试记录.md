...
```typescript
// 降序排序：边贡率越高越好，最高的在前面
sortedByMarginRatio = items.sort(
  (a, b) => b.contributionMarginRatio - a.contributionMarginRatio
)
```
...
```typescript
// 升序排序：成本率越低越好，最低的在前面
sortedByCostRatio = items.sort(
  (a, b) => a.variableCostRatio - b.variableCostRatio
)
```
