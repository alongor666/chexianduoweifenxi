...
## Issue #006: 新旧 Store 数据不同步导致上传后无数据显示
**状态**: ✅ 已解决  
**发现时间**: 2025-11-30  
**解决时间**: 2025-11-30  

### 问题描述
CSV 导入成功后，页面仍停留在“上传数据文件以开始分析”，表现为主界面读取的新架构数据源为空。

### 根本原因
架构迁移期间存在“双数据源并行”：
- 页面侧通过 `useInsuranceData()`（新 Domain Store）读取数据
- 上传侧仍在更新旧的 `useAppStore`，导致新旧状态不一致

### 解决方案
将“写入 rawData”逻辑统一封装，并在写旧 Store 的同时同步写新 Store。

**关键代码**：
- `src/store/utils/data-operations.ts`
  - `setRawDataLogic()`：写入旧 Store 并同步 `useDataStore.getState().setData(...)`
  - `appendRawDataLogic()`：追加写入并同步新 Store
