...
> - **技术ID**：Domain 层 camelCase；**上传字段名**：CSV 原始名（snake_case）；**UI 名**：前端展示名；**英文名/别名**：用于跨系统或历史表述的映射，避免匹配错误。
...
| commercialPremiumBeforeDiscountYuan | commercial_premium_before_discount_yuan | 商业险自主系数 | premium before discount | Number / 元 | 折前商业险保费，≥0 | 聚合：sum；维度同上 |
...
| 技术ID | UI 名 | 英文名/别名 | 计算依赖 | 类型/单位 | 聚合方式/维度 | 备注 |
| --- | --- | --- | --- | --- | --- | --- |
| maturedPremium | 满期保费（校验口径） | matured premium (derived) | maturedPremiumYuan | Number / 元 | sum | 与原始字段一致，缺失时为0参与计算 |
| commercialPremiumBeforeDiscount | 商业险自主系数 | commercial premium before discount | commercialPremiumBeforeDiscountYuan | Number / 元 | sum | 与原始字段一致 |
| premiumAchievementRate | 保费达成率 | premium achievement rate | signedPremiumYuan, premiumPlanYuan | Percent | avg/最新 | 目标缺失返回 null |
| policyProgressAchievementRate | 件数时间进度达成率 | policy progress achievement | policyCount, policyPlanCount | Percent | avg/最新 | 目标缺失返回 null |
| marginalContributionRate | 满期边际贡献率 | marginal contribution rate | variableCostRate | Percent | avg | 100% - 变动成本率 |
| maturedLossRatio | 满期赔付率 | matured loss ratio | reportedClaimPaymentYuan, maturedPremiumYuan | Percent | avg | 防御式分母检查 |
| expenseRatio | 费用率 | expense ratio | expenseAmountYuan, signedPremiumYuan | Percent | avg | 分母≤0返回 null |
| variableCostRate | 变动成本率 | variable cost rate | maturedLossRatio, expenseRatio | Percent | avg | 相加求和 |
| maturedContributionAmount | 满期边际贡献额 | matured contribution amount | maturedPremiumYuan, marginalContributionRate | Number / 元 | sum | 若已有原始值则以公式校验并可覆盖缺失 |
| maturedRate | 满期率 | matured rate | maturedPremiumYuan, signedPremiumYuan | Percent | avg | 满期保费/签单保费 |
| maturedClaimFrequency | 满期出险率 | matured claim frequency | claimCaseCount, policyCount | Percent | avg | 件数不足返回 null |
| policyAvgPremium | 单均保费 | avg premium per policy | signedPremiumYuan, policyCount | Number / 元 | avg | |
| claimAvgPayment | 案均赔款 | avg claim payment | reportedClaimPaymentYuan, claimCaseCount | Number / 元 | avg | |
| expenseAvg | 单均费用 | avg expense per policy | expenseAmountYuan, policyCount | Number / 元 | avg | |
| marginalContributionAvg | 单均边贡额 | avg marginal contribution | maturedContributionAmount, policyCount | Number / 元 | avg | |
| premiumProgressAchievementRate | 保费时间进度达成率 | premium progress achievement | signedPremiumYuan, premiumPlanYuan, weekNumber | Percent | avg/最新 | 支持累计(A)与单周(B)模式 |
| policyProgressTarget | 年度件数目标 | policy_plan_count | 业务规划 | Integer | max/latest | 如无规划为 null |
| premiumProgressTarget | 年度保费目标 | premium_plan_yuan | premiumPlanYuan | Number / 元 | max/latest | 同上 |
| weekOverWeekGrowth | 环比增长率 | WoW growth | currentValue, lastWeekValue | Percent | avg | 缺上周则 null |
| yearOverYearGrowth | 同比增长率 | YoY growth | currentValue, lastYearValue | Percent | avg | 缺去年同期则 null |
...
命名层次
- Domain 层 camelCase，Types/数据库层 snake_case；通过映射工具转换，禁止重复实现计算逻辑。
