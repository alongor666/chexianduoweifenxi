...
2.  **变动成本率 (成本控制能力)**
    - **公式**: `满期赔付率 + 费用率 = (已报告赔款 / 满期保费) + (费用金额 / 签单保费)`
    - **解读**: 业务的直接成本结构。
    - **⚠️ 重要**: 率值必须基于绝对值聚合，不能简单地将分子相加后除以单一分母，因为满期赔付率和费用率的分母不同（满期保费 vs 签单保费）。
...
为了支持按二级或三级机构分别计算保费时间进度达成率，系统引入机构年度计划数据源：

#### 4.3.1 数据源

**文件位置**: `src/data/reference/year-plans.json`
...
**覆盖范围**:
- **二级机构**: 四川（全覆盖）
- **三级机构**: 本部、天府、武侯、高新、青羊、新都、德阳、宜宾、泸州、乐山、自贡、资阳、达州（共13个）

#### 4.3.2 计算逻辑扩展

机构维度的保费时间进度达成率计算复用现有的两种模式，但目标数据来源不同：

**二级机构模式**:
```
年度目标 = 该二级机构下所有三级机构目标之和
时间进度达成率计算公式 = 与整体计算相同
```
...
**关键实现要点**:
1. **数据筛选**: 根据机构级别筛选对应的保险记录
2. **目标匹配**: 从year-plans.json中获取对应机构的年度目标
3. **计算复用**: 调用现有`calculateKPIs`函数，传入机构专属目标
4. **缓存机制**: 利用现有KPI引擎缓存，按机构维度缓存结果
...
> **核心原则**: 所有表格、图表、文字呈现必须按照"**从最差到最好**"的统一顺序排序，帮助用户快速识别问题和优秀表现。

#### 排序规则

| 指标类型 | 排序方向 | 示例指标 | 排序结果示例 |
|---------|---------|---------|-------------|
| **正向指标**<br>（越高越好） | **由低到高** | 边际贡献率、时间进度达成率、单均保费 | [70%, 80%, 90%, 100%]<br>从差→好 |
| **逆向指标**<br>（越低越好） | **由高到低** | 赔付率、费用率、变动成本率、案均赔款 | [100%, 90%, 80%, 70%]<br>从差→好 |

#### 指标分类完整清单

**正向指标（isHigherBetter = true）**：
- 时间进度达成率（`time_progress_ratio`）
- 边际贡献率（`contribution_margin_ratio`）
- 单均保费（`average_premium`）
- 满期率（`maturity_ratio`）

**逆向指标（isHigherBetter = false）**：
- 变动成本率（`variable_cost_ratio`）
- 满期赔付率（`loss_ratio`）
- 费用率（`expense_ratio`）
- 满期出险率（`matured_claim_ratio`）
- 案均赔款（`average_claim`）
- 单均费用（`average_expense`）
...
**核心函数**: `sortByValue(items, getValue, metricKey)`
- 自动根据指标的 `isHigherBetter` 属性决定排序方向
- 正向指标：升序排列（数值从小到大）
- 逆向指标：降序排列（数值从大到小）

**使用示例**:
```typescript
import { sortByValue } from '@/config/thresholds'

// 自动按"从差到好"排序
const sortedOrgs = sortByValue(
  organizations,
  org => org.kpi.loss_ratio,  // 赔付率（逆向指标）
  'loss_ratio'
)
// 结果：赔付率高的机构（差）排在前面，赔付率低的机构（好）排在后面
```

#### 应用场景

本规范必须应用于所有可视化场景：
- **驾驶舱**：所有图表（柱状图、条形图、热力图等）的数据排序
- **对比分析**：机构对比、业务类型对比表格
- **风险预警**：落后机构列表、超标业务列表
- **文字说明**：如"排名前5的机构"必须指最差的5个

#### 遵循此规范的优势

1. **用户体验一致**：所有页面的排序逻辑统一，降低用户学习成本
2. **问题快速定位**：差的数据总在最前面，便于快速识别和处理
3. **代码可维护性**：配置驱动，避免重复逻辑和硬编码
4. **减少错误**：统一工具函数，避免手动排序导致的方向错误
...
- **与代码一致性**: ✅ **完全一致**（已验证）
- **重要更新**:
  - ✅ **2025-12-16**: 修复变动成本率计算逻辑，确保率值基于绝对值聚合（满期赔付率 + 费用率），而非简单的分子相加
...
KPI计算支持按多个维度进行聚合和筛选，各维度的有效枚举值如下：

### 业务类型（16种标准类型）

**字段**: `business_type_category`
...
**兼容形式**: 支持11种变体形式（如 `10吨以上-普货` 自动映射到 `10吨以上营业货车（普货）`）

**详细定义**: 参见 [data_architecture.md - 业务类型枚举值详细说明](./data_architecture.md#业务类型枚举值详细说明)

### 客户类型（11种）

**字段**: `customer_category_3`

```
挂车、摩托车、特种车、营业公路客运、营业出租租赁、
营业城市公交、营业货车、非营业个人客车、非营业企业客车、
非营业机关客车、非营业货车
```

**重要**: 客户类型与业务类型是**完全不同的两个维度**，不可混淆！
...
**字段**: `third_level_organization`

```
本部、达州、德阳、高新、乐山、泸州、青羊、
天府、武侯、新都、宜宾、资阳、自贡
```
...
- **[数据架构文档](./data_architecture.md)**: 详细的数据结构规范、字段定义和枚举值完整列表
