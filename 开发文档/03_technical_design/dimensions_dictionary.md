...
评级维度包含四种不同的评级类型，每种评级类型对应特定的车辆类型：

#### 1. 车险分等级（`vehicle_insurance_grade`）
- **枚举值**：A | B | C | D | E | F | G | X
- **适用车型**：客车类型
- **适用条件**：
  - 客户类别：非营业个人客车、非营业企业客车、非营业机关客车、营业出租租赁、营业公路客运、营业城市公交
  - 业务类型：非营业客车新车、非营业客车旧车非过户、非营业客车旧车过户车、出租车、网约车

#### 2. 高速风险等级（`highway_risk_grade`）
- **枚举值**：A | B | C | D | E | F | X
- **适用车型**：客车类型
- **适用条件**：与车险分等级相同（客车）

#### 3. 小货车评分（`small_truck_score`）
- **枚举值**：A | B | C | D | E | X
- **适用车型**：9吨以下货车
- **适用条件**：
  - 业务类型：2吨以下营业货车、2-9吨营业货车、非营业货车新车、非营业货车旧车

#### 4. 大货车评分（`large_truck_score`）
- **枚举值**：A | B | C | D | E | X
- **适用车型**：9吨以上货车
- **适用条件**：
  - 业务类型：9-10吨营业货车、10吨以上-普货、10吨以上-牵引、自卸

**注意**：
- 各评级字段允许为空（可选字段）
- X 表示未评级状态
- 筛选器中根据已选择的客户类别或业务类型，动态显示对应的评级筛选项
