...
test.describe("评级筛选器条件显示", () => {
  ...
  test("初始状态：未选择任何筛选条件时，所有评级选项都应显示", async ({
    page,
  }) => {
    // 打开"更多筛选"对话框
  ...
  test("选择客车类别后，应只显示客车相关评级", async ({ page }) => {
    ...
    // 选择客户分类：非营业个人客车
    const customerCategoryButton = page.locator(
      'button:has-text("选择客户分类")',
    );
  ...
  test("对话框应该只有一个关闭按钮", async ({ page }) => {
    ...
    const closeButtons = page.locator(
      '[role="dialog"] button[aria-label="Close"], [role="dialog"] button:has(svg):has-text("")',
    );
